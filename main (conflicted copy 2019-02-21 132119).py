...
PRECURSOR_DATA = l_ds_Sem
PRECURSOR_LONGITIUDE = PRECURSOR_DATA[0].longitude.values
PRECURSOR_LATITUDE = PRECURSOR_DATA[0].latitude.values
PRECURSOR_LAGS = PRECURSOR_DATA[0].lag.values
PRECURSOR_YEARS = [np.unique(test['RV'].time.dt.year) for test in ex['train_test_list'][:][1]][0]
...
for test_idx, year in enumerate(PRECURSOR_YEARS):
    indices = l_ds_Sem[test_idx].pat_num_CPPA.values
    for lag_idx, lag in enumerate(pattern.lag.values):
        print(lag)
